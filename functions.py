...
def get_pair_factors_of_number(length_of_vocabulary):
    ...
    pairs = []
    given_list = [i for i in range(1, 12)] + [13, 17, 19]
...
def smile_to_hot_reduced(input, vocabulary_size, reduction_type):
    """
    Go from a single smile string to a one-hot encoding.
    """
    # one hot-encode input smile
    onehot_encoded = []
    for valueTensor in input:
        value = valueTensor.item()
        if reduction_type == 'reduction1':
            pairs = vocabulary_size
            letter = [0 for _ in range(pairs[0])]
            new_position_index = value//pairs[1]
            equivalent_value = (1 + value - pairs[1]*new_position_index)/pairs[1]
            letter[new_position_index] = equivalent_value
        elif reduction_type == 'reduction2':
            pairs = vocabulary_size
            letter = [0 for _ in range(pairs[0])]
            new_position_index = value//pairs[1]
    ...
    return torch.tensor(onehot_encoded).unsqueeze(1)
